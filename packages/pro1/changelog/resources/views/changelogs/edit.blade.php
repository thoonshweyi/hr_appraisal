...
<header class="mb-8">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-gray-800 header-gradient">Edit Change Log</h1>
            <p class="text-gray-600 mt-2">Announce system updates to all users</p>
        </div>
        <div class="flex items-center space-x-4">
            <button id="updateBtn" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="form-section bg-white rounded-xl shadow-md p-6">
                    ...
                    <div class="mt-4">
                        ...
                        <input name="title" type="text" id="releaseTitle" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" placeholder="e.g., Enhanced Analytics & Dark Mode" required value="{{ old('title',$changelog->title) }}">
                    </div>

                    <div class="mt-4">
                        <label class="block text-sm font-medium text-gray-700 mb-2">Description <span class="text-red-600">*</span></label>
                        <textarea name="description" id="releaseDescription" rows="4" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" placeholder="Brief description of this release..." required>{{ old('description',$changelog->description) }}</textarea>
                    </div>
                    ...
                    <div class="mt-4">
                        ...
                        <select id="releaseType" name="release_type_id" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" required>
                            ...
                            @foreach ($releasetypes as $releasetype)
                                <option value="{{ $releasetype->id }}" {{ $releasetype->id == old('release_type_id',$changelog->release_type_id) ? 'selected' : '' }}>{{ $releasetype->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                        <div>
                            ...
                            <input type="text" name="version_number" id="versionNumber" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" placeholder="e.g., 2.1.0" required value="{{ old('version_number',$changelog->version_number) }}">
                        </div>
                        ...
                        <div>
                            ...
                            <input
                                ...
                                required
                                value="{{ \Carbon\Carbon::parse($changelog->release_date)->format('Y-m-d') }}"
                            >
                        </div>
                    </div>
                </div>
                        ...
                        <div>
                            ...
                            <select id="priorityLevel" name="priority_level_id" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                                ...
                                @foreach ($prioritylevels as $prioritylevel)
                                    <option value="{{ $prioritylevel->id }}" {{ $prioritylevel->id == old('priority_level_id',$changelog->priority_level_id) ? 'selected' : '' }}>{{ $prioritylevel->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <select id="releaseStatus" name="status_id" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                                ...
                                @foreach ($statuses as $status)
                                    <option value="{{ $status->id }}" {{ $status->id == old('status_id',$changelog->status_id) ? 'selected' : '' }}>{{ $status->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div class="form-section bg-white rounded-xl shadow-md p-6">
                    ...
                    <div class="space-y-4 h-[500px] overflow-y-auto">
                        <div class="flex items-center">
                            <input type="checkbox" id="role-all" class="role-checkbox rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" value="all"
                            {{ $roles->count() === count($changelog->roles()->pluck('roles.id')->toArray()) ? 'checked' : ''}}
                            >
                        ...
                        @foreach ($roles as $role)
                            ...
                            @endphp
                        <div class="flex items-center">
                            {{-- @dd($changelog->roles()->pluck('roles.id')->toArray()) --}}
                            ...
                            {{ in_array($role->id,$changelog->roles()->pluck('roles.id')->toArray()) ? 'checked' : ''  }}
                            >
...
.media-preview:hover .preremovebtn {
    opacity: 1;
}
</style>
@endsection
...
$(document).ready(function(){
    ...
    const baseUrl = "{{ url('/api/changelogs/' . $changelog->id . '/changes') }}";
    ...
    $('#saveChangeBtn').click(async function(){
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                const change_ids = changefiles.map((changefile)=>changefile.id);
                ...
                formData.append(`title`, change.title);
                formData.append(`description`, change.description);
                formData.append(`change_type_id`, change.change_type_id);
                formData.append(`user_id`, change.user_id);

                change_ids.forEach((changeid)=>{
                    formData.append(`change_ids[]`, changeid);
                })


                change.mediafiles.forEach((file, j) => {
                    formData.append(`mediafiles[${j}]`, file);
                });
                ...
                if (currentEditIndex >= 0) {
                    ...
                    changelogurl = `${baseUrl}/${currentEditIndex}`;
                    changelogmethod = "PUT";
                    formData.append('_method', 'PUT');

                } else {
                    changelogurl = baseUrl;
                    changelogmethod = "POST";
                }



                {{-- console.log(changelogurl); --}}
                await fetch(changelogurl, {
                    method: "POST",
                    body: formData
                })
                .then(res => res.json())
    ...
    function getActiveType() {
        ...
        return active ? active.getAttribute('data-change-type') : '';
    }
            ...
            reader.onload =  function(e) {
                ...
                if (file.type.startsWith('image/')) {
                    div.innerHTML = `
                        ...
                        <button type="button" class="remove-btn preremovebtn" data-index="${index}">
                            <i class="fas fa-times"></i>
                        </button>
                    `;
                } else if (file.type.startsWith('video/')) {
                    div.innerHTML = `
                        <a href="${fileURL}" data-fancybox="change-changelogfile" data-caption="Change Media"><video src="${fileURL}" class="w-full h-20 object-cover rounded" controls ></video></a>
                        <button type="button" class="remove-btn preremovebtn"  data-index="${index}">
                            <i class="fas fa-times"></i>
                        </button>
                    `;
    ...
    $('#mediaPreview').on('click', '.removechangefile-btn', function () {
        const index = $(this).data('index');
        changefiles.splice(index, 1);
        updateChangeMediaPreview(changefiles);
        updateMediaPreview();
    ...
    function clearMediaPreview(){
        $('#mediaPreview').children(':not(.changefile-preview)').remove();
    }
    {{-- End Media Upload --}}

    {{-- Start Update Change Media Preview --}}
    function updateChangeMediaPreview(changelogfiles) {
        const preview = document.getElementById('mediaPreview');
        preview.innerHTML = '';

        changelogfiles.forEach(async (file, index) => {

            const fileURL = file.mediafile;

            const div = document.createElement('div');
            div.className = 'media-preview relative changefile-preview';

            if (isImageFile(fileURL)) {
                div.innerHTML = `
                    <a href="{{asset('${fileURL}')}}" data-fancybox="change-changelogfile" data-caption="Change Media"><img src="{{asset('${fileURL}')}}" alt="${file.name}" class="w-full h-20 object-cover rounded"></a>
                    <button type="button" class="removechangefile-btn preremovebtn" data-index="${index}">
                        <i class="fas fa-times"></i>
                    </button>
                `;
            } else if (isVideoFile(fileURL)) {
                div.innerHTML = `
                    <a href="{{asset('${fileURL}')}}" data-fancybox="change-changelogfile" data-caption="Change Media"><video src="{{asset('${fileURL}')}}" class="w-full h-20 object-cover rounded" controls  data-index="${index}"></video></a>
                    <button type="button" class="removechangefile-btn preremovebtn">
                        <i class="fas fa-times"></i>
                    </button>
                `;
            }
        ...
        changes.forEach((change,idx)=>{
            ...
            <div class="change-item ${change.changetype.slug} p-4 bg-gray-50 rounded-xl shadow-sm hover:shadow-md transition-all">
                ...
                <div class="flex space-x-2 justify-end">
                    <button type="button" class="edit-change-btn text-blue-600 hover:text-blue-800" data-changeid="${change.id}">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button type="button" class="delete-change-btn text-red-600 hover:text-red-800" data-changeid="${change.id}">
                        <i class="fas fa-trash"></i>
                    </button>
                </div>

                <div class="change-media grid grid-cols-4 gap-2">
                      ${change.changelogfiles.map(file => {
                        const url = file.mediafile;


                        if (isImageFile(url)) {
                            return `<a href="{{asset('${url}')}}" data-fancybox="change${change.id}-changelogfile" data-caption="Change ${idx+1} Media"><img src="{{asset('${url}')}}" alt="${file.name}" class="w-full h-20 object-cover rounded" /></a>`;
                        } else if (isVideoFile(url)) {
                            return `<a href="{{asset('${url}')}}" data-fancybox="change${change.id}-changelogfile" data-caption="Change ${idx+1} Media"><video src="{{asset('${url}')}}" class="w-full h-20 object-cover rounded" controls></video></a>`;
                        }
                        return '';
                    }).join('')}
                </div>
    ...
    $(document).on('click','.edit-change-btn',function(){
        ...
        {{-- First Upload Existing Files --}}
        updateChangeMediaPreview(change.changelogfiles);


        mediafiles = [...change.mediafiles];
        changefiles = [...change.changelogfiles];
        updateMediaPreview();
    ...
    $('#updateBtn').click(function(){
        $.ajax({
            url:'{{url("/api/changelogs/$changelog->id/")}}',
            method:"PUT",
            ...
            success:function(response){
                ...
                if(data.errors){
                    ...
                    Swal.fire({
                        title: "Updated!",
                        text: "Change Log Updated Successfully",
                        icon: "success"
                    });
                }
