        ...
        <div class="table-responsive rounded mb-3">
            <table class="table mb-0" id="branch_list">
                ...
                <tbody class="ligth-body">
                    @foreach($deptgroups as $idx=>$deptgroup)
                    <tr>
                        <td>{{$idx + $deptgroups->firstItem()}}</td>
                        <td>{{$deptgroup["name"]}}</td>
                        <td>
                            <div class="custom-switch p-0">
                                <!-- The actual checkbox that controls the switch -->
                                <input type="checkbox" id="customSwitch-{{ $idx + $deptgroups->firstItem() }}" class="custom-switch-input statuschange-btn" {{ $deptgroup->status_id === 1 ? "checked" : "" }} data-id="{{ $deptgroup->id }}"/>
                                <!-- The label is used to style the switch, and clicking it toggles the checkbox -->
                                <label class="custom-switch-label" for="customSwitch-{{ $idx + $deptgroups->firstItem() }}"></label>
                                <!-- Optional label text next to the switch -->
                            </div>
                        </td>
                        <td>{{ $deptgroup["user"]["name"] }}</td>
                        <td>{{ $deptgroup->created_at->format('d M Y') }}</td>
                        <td>{{ $deptgroup->updated_at->format('d M Y') }}</td>
                        <td class="text-center">
                             <a href="javascript:void(0);" class="text-info editform mr-2" data-toggle="modal" data-target="#editmodal" data-id="{{$deptgroup->id}}" data-name="{{$deptgroup->name}}" data-status="{{$deptgroup->status_id}}"><i class="fas fa-pen"></i></a>
                             <a href="#" class="text-danger ms-2 delete-btns" data-idx="{{$idx}}"><i class="fas fa-trash-alt"></i></a>
                        </td>
                        <form id="formdelete-{{ $idx }}" class="" action="{{route('deptgroups.destroy',$deptgroup->id)}}" method="POST">
                             @csrf
                             @method("DELETE")
                        </form>
                   </tr>
        ...
        <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content rounded-0">
                    ...
                    <div class="modal-body">
                        <form id="{{route('deptgroups.store')}}" action="" method="POST">
                            {{ csrf_field() }}
                            <div class="row align-items-end">
                                <div class="col-md-12">
...
<script>
    $(document).ready(function() {
       ...
       $(document).on("click",".editform",function(e){
            ...
            $("#formaction").attr("action",`/deptgroups/${getid}`);
