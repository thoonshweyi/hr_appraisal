...
<div class="content-page">
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="col-lg-12">
                <form class="d-inline" action="{{ route('subsections.excel_import') }}" method="POST" enctype="multipart/form-data">
                    @csrf
            ...
            <div class="col-lg-12 my-2 ">
                <form class="d-inline" action="{{ route('subsections.index') }}" method="GET">
                    @csrf
                    <div class="row align-items-end">

                        <div class="col-md-2">
                            <label for="filter_name">Name <span class="text-danger">*</span></label>
                            <input type="text" name="filter_name" id="filter_name" class="form-control form-control-sm rounded-0" placeholder="Enter Sub Section Name" value="{{ request()->filter_name }}"/>
                        </div>
        ...
        <div class="table-responsive rounded mb-3">
            <table class="table mb-0" id="branch_list">
                ...
                <tbody class="ligth-body">
                    ...
                    <tr>
                        <td>{{$idx + $subsections->firstItem()}}</td>
                        <td>{{$subsection["name"]}}</td>
                        {{-- <td>{{ $subsection->division->name }}</td>
                        <td>{{ $subsection->department->name}}</td>
                        <td>{{ $subsection->subdepartment->name}}</td> --}}
                        <td>
                            <div class="custom-switch p-0">
                                <!-- The actual checkbox that controls the switch -->
                                <input type="checkbox" id="customSwitch-{{ $idx + $subsections->firstItem() }}" class="custom-switch-input statuschange-btn" {{ $subsection->status_id === 1 ? "checked" : "" }} data-id="{{ $subsection->id }}"/>
                                <!-- The label is used to style the switch, and clicking it toggles the checkbox -->
                                <label class="custom-switch-label" for="customSwitch-{{ $idx + $subsections->firstItem() }}"></label>
                                <!-- Optional label text next to the switch -->
                            </div>
                        </td>
                        <td>{{ $subsection["user"]["name"] }}</td>
                        <td>{{ $subsection->created_at->format('d M Y') }}</td>
                        <td>{{ $subsection->updated_at->format('d M Y') }}</td>
                        <td class="text-center">
                            @can('edit-fixed-analysis')
                            <a href="javascript:void(0);" class="text-info editform mr-2" data-toggle="modal" data-target="#editmodal" data-id="{{$subsection->id}}" data-name="{{$subsection->name}}" data-code="{{$subsection->code}}" data-division="{{$subsection->division_id}}" data-department="{{$subsection->department_id}}" data-sub_department="{{$subsection->sub_department_id}}" data-status="{{$subsection->status_id}}"><i class="fas fa-pen"></i></a>
                            @endcan
                            @can('delete-fixed-analysis')
                            <a href="#" class="text-danger ms-2 delete-btns" data-idx="{{$idx}}"><i class="fas fa-trash-alt"></i></a>
                            @endcan
                        </td>
                        <form id="formdelete-{{ $idx }}" class="" action="{{route('subsections.destroy',$subsection->id)}}" method="POST">
                             @csrf
                             @method("DELETE")
                        </form>
                   </tr>
            ...
            <div class="d-flex justify-content-center">
                {{ $subsections->appends(request()->all())->links("pagination::bootstrap-4") }}
            </div>
        ...
        <div class="modal-dialog modal-lg modal-dialog-centered">
                <div class="modal-content rounded-0">
                    ...
                    <div class="modal-body">
                        <form id="{{route('subsections.store')}}" action="" method="POST">
                            {{ csrf_field() }}
                            <div class="row align-items-end">
                                <div class="col-md-6">
...
<script>
    $(document).ready(function() {
       ...
       $(document).on("click",".editform",function(e){
            ...
            $("#formaction").attr("action",`/subsections/${getid}`);
