        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="d-flex flex-wrap flex-wrap align-items-center justify-content-between mb-4">
                    <div>
                        <h4 class="mb-3">Position Levels</h4>
            ...
            {{-- <div class="col-lg-12">
                <form class="d-inline" action="{{ route('positionlevels.excel_import') }}" method="POST" enctype="multipart/form-data">
                    @csrf
        ...
        <div class="table-responsive rounded mb-3">
            <table class="table mb-0" id="branch_list">
                ...
                <tbody class="ligth-body">
                    @foreach($positionlevels as $idx=>$positionlevel)
                    <tr>
                        <td>{{$idx + $positionlevels->firstItem()}}</td>
                        <td>{{$positionlevel["name"]}}</td>
                        <td>
                            <div class="custom-switch p-0">
                                <!-- The actual checkbox that controls the switch -->
                                <input type="checkbox" id="customSwitch-{{ $idx + $positionlevels->firstItem() }}" class="custom-switch-input statuschange-btn" {{ $positionlevel->status_id === 1 ? "checked" : "" }} data-id="{{ $positionlevel->id }}"/>
                                <!-- The label is used to style the switch, and clicking it toggles the checkbox -->
                                <label class="custom-switch-label" for="customSwitch-{{ $idx + $positionlevels->firstItem() }}"></label>
                                <!-- Optional label text next to the switch -->
                            </div>
                        </td>
                        <td>{{ $positionlevel["user"]["name"] }}</td>
                        <td>{{ $positionlevel->created_at->format('d M Y') }}</td>
                        <td>{{ $positionlevel->updated_at->format('d M Y') }}</td>
                        <td class="text-center">
                            @can('edit-fixed-analysis')
                                <a href="javascript:void(0);" class="text-info editform mr-2" data-toggle="modal" data-target="#editmodal" data-id="{{$positionlevel->id}}" data-name="{{$positionlevel->name}}" data-status="{{$positionlevel->status_id}}"><i class="fas fa-pen"></i></a>
                            @endcan
                            @can('delete-fixed-analysis')
                             <a href="#" class="text-danger ms-2 delete-btns" data-idx="{{$idx}}"><i class="fas fa-trash-alt"></i></a>
                            @endcan
                        </td>
                        <form id="formdelete-{{ $idx }}" class="" action="{{route('positionlevels.destroy',$positionlevel->id)}}" method="POST">
                             @csrf
                             @method("DELETE")
                        </form>
                   </tr>
            ...
            <div class="d-flex justify-content-center">
                {{ $positionlevels->appends(request()->all())->links("pagination::bootstrap-4") }}
            </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content rounded-0">
                    ...
                    <div class="modal-body">
                        <form id="{{route('positionlevels.store')}}" action="" method="POST">
                            {{ csrf_field() }}
                            <div class="row align-items-end">
                                <div class="col-md-12">
...
<script>
    $(document).ready(function() {
       ...
       $(document).on("click",".editform",function(e){
            ...
            $("#formaction").attr("action",`/positionlevels/${getid}`);
